...
* ts_rank(volume, 5))
```
**逻辑**：5 日最低价的 5 日变化（取负）$\times$ 长期收益减短期收益的排名 $\times$ 成交量 5 日时序排名。
**类型**：动量 + 成交量
...
```
**逻辑**：低价与收盘价之差（通常为负）$\times$ 开盘价五次方，除以日内振幅 $\times$ 收盘价五次方，取负。
**类型**：价格结构
...
```
**逻辑**：10 日收益与短期收益之比的排名 $\times$ 收益与市值乘积的排名，取负。
**类型**：动量 + 市值
...
```
**逻辑**：$2 \times$ 日内位置加权成交量的缩放排名，减去 10 日最高收盘价出现时间的缩放排名，取负。
**类型**：量价结构
