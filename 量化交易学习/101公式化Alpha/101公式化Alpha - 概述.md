...
| 类型 | 逻辑 | 示例 |
|------|------|------|
| **均值回归 (Mean-Reversion)** | 信号方向与收益方向相反，押注价格回归 | $-\ln(\text{今日开盘} / \text{昨日收盘})$ |
| **动量 (Momentum)** | 信号方向与收益方向相同，押注趋势延续 | $\ln(\text{昨日收盘} / \text{昨日开盘})$ |
...
1. **收益 ~ 波动率**：年化平均日收益与日收益波动率满足幂律关系 $\mu \sim \sigma^{0.76}$
2. **换手率解释力弱**：换手率对 Alpha 两两相关性的解释力很差（$R^2 \approx 1.27\%$）
3. **换手率与波动率有一定相关性**：$\ln(\sigma) \approx -6.174 + 0.368 \times \ln(\tau)$
...
| `decay_linear(x, d)` | 线性衰减加权移动平均（权重 $d, d{-}1, \ldots, 1$，归一化） |
| `scale(x, a)` | 缩放 $x$ 使得 $\sum\lvert x\rvert = a$（默认 $a=1$） |
